...
int64 nChainStartTime = 1404518400;
...
const string strMessageMagic = "Architectcoin Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if (nHeight > 0 && nHeight < 1441)
    {
	nSubsidy = 3472.223 * COIN;
    }
                                   // Next 50 blocks used to confrim first IPO block
                                   // POW is cutoff in the CBlock:: code. For safety, PoW blocks are pemitted till block 15000
...
const int YEARLY_BLOCKCOUNT = 525600 ;	// 365 * 1440 - block every minute
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake) 
{
    ...
    if (fProofOfStake || pindexLast->nHeight > 300)
    {
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Architectcoin
    bool fStrictPayToScriptHash = true; // Always active in Architectcoin
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    if (!CheckStakeModifierCheckpoints(pindexNew->nHeight, pindexNew->nStakeModifierChecksum))
        return error("AddToBlockIndex() : Rejected by stake modifier checkpoint height=%d, modifier=0x%016"PRI64x", checksum=%d", pindexNew->nHeight, nStakeModifier, pindexNew->nStakeModifierChecksum);
...
bool CBlock::AcceptBlock()
{
    ...
    // End POW after we mine block 15000
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Architectcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "RT 11/07/2014 - US Navy unveils electromagnetic railgun prototypes";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1404518461;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 6277945;
        ...
        assert(block.hashMerkleRoot == uint256("0xc3788ac6a7439d26e495737504d28556467066c01b2f2c9cfdfe3e4358042bce"));
...
unsigned char pchMessageStart[4] = { 0xa4, 0x71, 0xf1, 0x9f };
