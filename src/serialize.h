...
template<typename Stream>
uint64 ReaArchitectcoinpactSize(Stream& is)
{
    ...
    if (nSizeRet > (uint64)MAX_SIZE)
        THROW_WITH_STACKTRACE(std::ios_base::failure("ReaArchitectcoinpactSize() : size too large"));
    return nSizeRet;
}
...
{
    unsigned int nSize = ReaArchitectcoinpactSize(is);
    str.resize(nSize);
    if (nSize != 0)
