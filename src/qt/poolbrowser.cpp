...
const QString apiBittrexTrades = "http://bittrex.com/api/v1/public/getmarkethistory?market=BTC-ARCH&count=100";
const QString apiBittrexOrders = "http://bittrex.com/api/v1/public/getorderbook?market=BTC-ARCH&type=both&depth=50";
...
{
    QDesktopServices::openUrl(QUrl("https://www.bittrex.com/Market/Index?MarketName=BTC-ARCH"));
}
void PoolBrowser::openPoloniex()
{
    QDesktopServices::openUrl(QUrl("https://poloniex.com/exchange/btc_arch"));
}
...
void PoolBrowser::bittrexMarketSummary(QNetworkReply* response)
{
    ...
    for(int i = 0; i < qslApiResponse.count(); i++){
        ...
        if(read_string(qslApiResponse[i].toStdString(), jsonResponse)) {
            ...
            if (getPairValue(jsonObject, "MarketName").get_str() == "BTC-ARCH") {
                try {
